...
class TaskEdgeCases(TestCase):
    ...
    def test_create_task_with_long_description(self):
        ...
        with self.assertRaises(ValidationError):
            task = Task(title='Title', description=long_description, Due_date=date.today())
            task.full_clean()
    ...
    def test_create_task_with_past_due_date(self):
        ...
        with self.assertRaises(ValidationError):
            task = Task(title='Title', description='Description', Due_date=past_due_date)
            task.full_clean()
    ...
    def test_create_task_with_invalid_status_choice(self):
        ...
        with self.assertRaises(ValidationError):
            task = Task(title='Title', description='Description', Due_date=date.today(), status='INVALID_STATUS')
            task.full_clean()
